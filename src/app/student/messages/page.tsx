  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-500 mt-1">Your conversations with tutors.</p>
      </div>
      ...
      {threads.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-2xl border border-slate-100">
          ...
          <p className="text-slate-500 text-sm mb-5">Message a tutor from their profile to get started.</p>
          <Link
            href="/search"
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm divide-y divide-slate-100 overflow-hidden">
          {threads.map((t) => (
            <Link
              key={t.threadId}
              href={`/student/messages/${t.threadId}`}
